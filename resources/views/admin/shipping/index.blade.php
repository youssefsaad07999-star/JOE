    ...
    <div x-data="{ open: false }" @open-add-shipping.window="open = true">
        ...
        <div x-show="open" x-transition @click.self="open = false"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md" @click.stop>
                ...
                <form action="{{ route('admin.shipping.store') }}" method="POST" class="p-6 space-y-4">
                    @csrf
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Method
                    </th>
                    <th class="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Delivery
                        Time</th>
                    <th class="text-right px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Price
                    </th>
                    <th class="text-center px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Sort
                    </th>
                    <th class="text-center px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Active
                    </th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors" x-data="{
                        ...
                        <td class="px-6 py-4">
                            ...
                            <input x-show="editing" type="text" x-model="name"
                                class="border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm w-44
                                          focus:outline-none focus:ring-1 focus:ring-[#C85C6E]"
                                style="display:none;">
                        </td>

                        <td class="px-6 py-4">
                            <span x-show="!editing" class="text-sm text-gray-500">{{ $method->delivery_time }}</span>
                            <input x-show="editing" type="text" x-model="delivery"
                                class="border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm w-40
                                          focus:outline-none focus:ring-1 focus:ring-[#C85C6E]"
                                style="display:none;">
                        </td>

                        <td class="px-6 py-4 text-right">
                            <span x-show="!editing" class="text-sm font-semibold">
                        ...
                        <td class="px-6 py-4 text-center">
                            <span x-show="!editing" class="text-sm text-gray-500">{{ $method->sort_order }}</span>
                            <input x-show="editing" type="number" x-model="sort" min="0"
                                class="w-16 border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm text-center
                        ...
                        <td class="px-6 py-4">
                            <div x-show="!editing" class="flex items-center justify-end gap-3">
                                <button @click="editing = true"
                                    class="text-xs text-gray-400 hover:text-[#C85C6E] transition-colors">
                                    Edit
                                </button>
                                <form action="{{ route('admin.shipping.destroy', $method) }}" method="POST" x-data
                            ...
                            <div x-show="editing" class="flex items-center justify-end gap-2" style="display:none;">
                                <form action="{{ route('admin.shipping.update', $method) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
