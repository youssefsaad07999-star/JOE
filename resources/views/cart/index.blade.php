    ...
    <div class="max-w-6xl mx-auto px-6 py-10">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <form action="{{ route('cart.destroy', ['cartItem' => $item]) }}" method="POST">
                                        ...
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" class="text-gray-300 hover:text-rose-500 transition-colors p-1">
                                ...
                                <div class="flex items-center justify-between mt-4">
                                    ...
                                    <div class="flex items-center border border-gray-200 rounded-full">
                                        ...
                                        <form action="{{ route('cart.update', ['cartItem' => $item]) }}" method="POST">
                                            ...
                                            <button type="submit" class="w-8 h-8 flex items-center justify-center text-gray-500
                                               hover:text-black transition text-lg" @if($item->quantity <= 1) disabled @endif>
                                                −
                                            </button>
                                        </form>
                                        ...
                                        <form action="{{ route('cart.update', ['cartItem' => $item]) }}" method="POST">
                                            ...
                                            <button type="submit" class="w-8 h-8 flex items-center justify-center text-gray-500
                                               hover:text-black transition text-lg" @if($item->quantity >= $item->variant->stock_quantity) disabled @endif>
                                                +
                                            </button>
                                        </form>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-2xl p-6 shadow-sm sticky top-24">
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            @if($cartTotal < 80)
                                <p class="text-xs text-gray-400 bg-gray-50 p-3 rounded-lg">
                                    Add ${{ number_format(80 - $cartTotal, 2) }} more for free shipping
                                </p>
                            @endif
                        </div>
                        ...
                        <div class="flex justify-between font-semibold text-lg mb-6">
                            <span>Total</span>
                            <span>
                                ${{ number_format($cartTotal + ($cartTotal < 80 ? 9.99 : 0), 2) }}
                            </span>
                        </div>
        ...
        @else
            ...
            <div class="text-center py-24">
                ...
                <a href="/" class="inline-block bg-[#1C1C1C] text-white px-8 py-3.5 rounded-full
